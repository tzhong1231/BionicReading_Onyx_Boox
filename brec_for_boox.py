...
def bolding_with_color(text, non_bold_color = 'grey'):
    ...
    for part in parts:
        if part in string.punctuation or part in string.digits:
            ...
            if len(part) <= 3:
                ...
                # The line below comments is the original code.
                # If you don't want the second half of the words to turn grey use this line instead.
                # Remember to delete the non_bold_color = 'grey' variable from the def in line 52
                # Also switch to original code in line 129
                # new_part += ''.join(part[point:])
                new_part += f'<span style="color: {non_bold_color};">{part[point:]}</span>'
                new_text += ' ' + new_part
    return new_text
...
for html in htmls:
    ...
    for html_part in data_html:
        ...
        if html_part[0] == 'Data:':
            ...
            # Below is the original code, use this line if you don't want grey
            #full_html += bolding_with_color(html_part[1])
            full_html += bolding_with_color(html_part[1], non_bold_color='grey')
